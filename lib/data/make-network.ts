...
import { values, DataProvider } from "simple-data-provider";
 ...
 */
export interface IMakeNetworkOptions<TNodeSource, TEdgeSource, TNodeMeta, TEdgeMeta> {
  /**
   ...
   */
  edgeIn: Accessor<TEdgeSource, Identifier, IEdge<TNodeMeta, TEdgeMeta>>;
  /**
   * The accessor to retrieve the id of the node the edge terminates at.
   * Provides an existing edge if the edge id already exists when
   * aggregateResults is true.
   */
  edgeOut: Accessor<TEdgeSource, Identifier, IEdge<TNodeMeta, TEdgeMeta>>;
 ...
 */
export interface IMakeNetworkResult<TNodeSource, TEdgeSource, TNodeMeta, TEdgeMeta>
  extends INetworkData<TNodeMeta, TEdgeMeta> {
  /** All errors discovered while processing the data from old to new format */
  errors: IMakeNetworkError<TNodeSource | INode<TNodeMeta, TEdgeMeta>, TEdgeSource | IEdge<TNodeMeta, TEdgeMeta>>[] | null;
}
 ...
 */
export async function makeNetwork<TNodeSource, TEdgeSource, TNodeMeta, TEdgeMeta>(
  ...
  const errors: IMakeNetworkError<TNodeSource | INode<TNodeMeta, TEdgeMeta>, TEdgeSource | IEdge<TNodeMeta, TEdgeMeta>>[] = [];
  ...
  const inToOutMap: INetworkData<TNodeMeta, TEdgeMeta>["inToOutMap"] = new Map();

  // This is the new network we're creating
  const network: IMakeNetworkResult<TNodeSource, TEdgeSource, TNodeMeta, TEdgeMeta> = {
    nodes,
    edges,
    nodeMap,
    edgeMap,
    ...
    for (let k = 0, kMax = ids.length; k < kMax; ++k) {
      ...
      if (aggregateResults) {
        ...
        if (previousPair) {
          ...
          if (nodeA) previous.in = nodeA;
          if (nodeB) previous.out = nodeB;
