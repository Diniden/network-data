 ...
 *
 * Specify removedEdges to prevent errors from being created across multiple
 * edge removals.
 */
export function removeEdge<TNodeMeta, TEdgeMeta>(
  ...
  for (let i = 0, iMax = edges.length; i < iMax; ++i) {
    ...
    // If the edge is not within the provided network, this is a no-op. This
    // also cleans the edge out of the network.
    ...
    const aIndex = edge.in.out.indexOf(edge);
    const bIndex = edge.out.in.indexOf(edge);
    if (aIndex > -1) edge.in.out.splice(aIndex, 1);
    if (bIndex > -1) edge.out.in.splice(bIndex, 1);
    ...
    removeFromMapOfMaps(network.inToOutMap, edge.in, edge.out);
  }
